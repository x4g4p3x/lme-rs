...
| 5 | **Rust crate: ANOVA & model comparison** — Type III: [`LmeFit::anova`](src/anova.rs); nested LRT: [`anova`](src/lib.rs) (`AnovaResult`) | **88%** | Type III only; 1-DoF marginal KR; joint multi-DoF Wald for grouped categoricals with Satterthwaite DenDF ([`src/ddf.rs`](src/ddf.rs)) and Kenward–Roger via [`pbkrtest::KRmodcomp`](src/kr_modcomp.rs) / [`vcovAdj16`](src/kr_vcov_adj.rs) ([`tests/test_kr_modcomp_pastes.rs`](tests/test_kr_modcomp_pastes.rs)). Not Type II, arbitrary contrasts, or full `car` / `lmerTest` superset. |
...
- **Fixed-effects ANOVA:** Type **III** only ([`README.md`](README.md)). **Type II** tables and arbitrary user-defined contrasts are **not** implemented. Multi-df **joint Wald** rows for grouped categorical dummies use Satterthwaite DenDF ([`src/ddf.rs`](src/ddf.rs)) and Kenward–Roger via full `pbkrtest::KRmodcomp` / `vcovAdj16` for covered LMMs ([`src/kr_modcomp.rs`](src/kr_modcomp.rs), [`src/kr_vcov_adj.rs`](src/kr_vcov_adj.rs)). General multi-df designs beyond grouped categoricals are not.
