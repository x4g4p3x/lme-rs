...
|---|------|:----------:|-------|
| 1 | **Rust crate: linear & mixed (LMM)** — [`lm`](src/lib.rs) / [`lm_df`](src/lib.rs), [`lmer`](src/lib.rs), [`lmer_weighted`](src/lib.rs), REML/ML, [`predict`](src/lib.rs) variants | **94%** | Core path is stable; parity-style checks in [`tests/test_numerical_parity.rs`](tests/test_numerical_parity.rs), e2e and optimization tests under [`tests/`](tests/). Fair-harness throughput on synthetic intercept/crossed/nested cases is within ~**2×** Julia on cold `lmer()` (2026-07-08) — see row 13 and [OPTIMIZATION.md](OPTIMIZATION.md). |
...
| 13 | **LMM fit throughput vs MixedModels.jl** — optimization to be **competitive** on fair harness cases | **~84%** | **Tier-A cases:** [BENCHMARK_COVERAGE.md](BENCHMARK_COVERAGE.md). **History:** `76fdb61` (~2–3× / ~8× / ~19×) → Jul 7 LDL (~8× crossed) → Jul 8 blocked Cholesky (~5×) → Jul 8 prepare/fast paths (~**1.5–1.6×** cold `lmer()` on synthetics; **`fit_prepared` ≈ Julia**). **Re-run required** after harness expansion (`sleepstudy`, 50k/100k, real fixtures, GLMM). **Target:** `cold_fit` ≤ **~2×** Julia per case. |
...
Synthetic fair-harness LMM throughput (intercept / crossed / nested) is **within ~2× Julia** on cold `lmer()` and **`fit_prepared` matches Julia** on crossed/nested on the 2026-07-08 reference workstation ([OPTIMIZATION.md](OPTIMIZATION.md)). Axis (3) is **not fully closed**: **`sleepstudy_reml`** (random slopes) and larger **`random_intercept_50k`/`100k`** cases were not re-benchmarked after the Jul 8 passes — benchmark on your RE layout before hot-path adoption ([USABILITY.md](USABILITY.md)).
