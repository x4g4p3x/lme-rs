...
|:------|:-------------|:------|
| **Nonlinear mixed models** (`nlmer`-style) | **~95%** | [`nlmer`](src/nlmm/mod.rs) + Python `nlmer()` / [`nlmer_with_mean`](python/src/lib.rs); built-in `SSlogis` / `SSasymp` / `SSfol` / `SSmicmen` / `SSgompertz` / **`SSpower`** / **`SSfpl`** / **`SSbiexp`** / **`SSweibull`**; optional population `lower`/`upper` box bounds; `selfStart` when `start` is empty (built-in means only); population/conditional [`predict`](src/nlmm/predict.rs); scalar and multivariate RE on one grouping factor; scalar AGQ **inside** the θ profile. **Not yet:** remaining `stats::SS*` (`SSasympOff` / `Orig`, …), multivariate AGQ for `k_re > 1`, per-group (β+b) bounds. |
