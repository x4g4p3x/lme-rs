...
|---|------|:----------:|-------|
| 1 | **Rust crate: linear & mixed (LMM)** — [`lm`](src/lib.rs) / [`lm_df`](src/lib.rs), [`lmer`](src/lib.rs), [`lmer_weighted`](src/lib.rs), REML/ML, [`predict`](src/lib.rs) variants | **93%** | Core path is stable; parity-style checks in [`tests/test_numerical_parity.rs`](tests/test_numerical_parity.rs), e2e and optimization tests under [`tests/`](tests/). **Not** complete for throughput: fair benchmark (2026-07-04) shows large gaps vs MixedModels.jl on crossed/nested fits — see row 13. |
...
| 13 | **LMM fit throughput vs MixedModels.jl** — optimization to be **competitive** on fair harness cases | **~42%** | 2026-07-04 Windows reference: Julia **5×–28×** faster by median fit time ([`benchmarks/fair-rust-julia-reference-2026-07-04.json`](benchmarks/fair-rust-julia-reference-2026-07-04.json)); largest gaps on crossed (`crossed_20k`) and nested (`nested_10k`) structures. Likely work: profile θ optimization / PIRLS hot paths, reduce allocations in design-matrix and sparse solves, improve crossed/nested RE handling, revisit derivative-free variance search vs gradient-based alternatives where parity allows. **Target:** within ~**2×** of MixedModels.jl medians on the six fair cases (stretch: parity). Re-run harness after each optimization pass. |

## Weighted “overall” (illustrative only)

Simple mean of the thirteen percentages:  
(93 + 88 + 87 + 86 + 92 + 97 + 93 + 84 + 98 + 90 + 76 + 35 + 42) ÷ 13 = 1074 ÷ 13 ≈ **82.6%**.

So the rough figure is **~83%** toward a hypothetical “full statistics stack with universal parity **and competitive native fit speed**.” That target is **not** fully met while row 13 remains ~40% — [`README.md`](README.md) describes the crate as usable today, but **performance-sensitive Rust workflows** should treat MixedModels/Julia parity on throughput as unfinished work until the fair harness medians move.
...
| Benchmarks / throughput | [`BENCHMARKS.md`](BENCHMARKS.md); [`scripts/run_fair_rust_julia_benchmark.py`](scripts/run_fair_rust_julia_benchmark.py); [`benchmarks/fair-rust-julia-reference-2026-07-04.json`](benchmarks/fair-rust-julia-reference-2026-07-04.json) |
| LMM throughput optimization backlog | Summary table row 13; [`benches/bench_math.rs`](benches/bench_math.rs) size/crossed/nested sweeps |
...
| **Competitive LMM fit vs MixedModels.jl** | **~42%** | Same as summary row 13. Documented regression on 2026-07-04; optimization not yet prioritized in the fitting stack. Completion criterion: fair-harness medians within ~**2×** of Julia on all six reference cases without breaking lme4 parity tests. |
