...
|---|------|:----------:|-------|
| 1 | **Rust crate: linear & mixed (LMM)** — [`lm`](src/lib.rs) / [`lm_df`](src/lib.rs), [`lmer`](src/lib.rs), [`lmer_weighted`](src/lib.rs), REML/ML, [`predict`](src/lib.rs) variants | **94%** | Core path is stable; parity-style checks in [`tests/test_numerical_parity.rs`](tests/test_numerical_parity.rs), e2e and optimization tests under [`tests/`](tests/). Throughput improved on random-intercept fair cases (2026-07-06 reference); crossed/nested gaps remain — see row 13. |
...
| 13 | **LMM fit throughput vs MixedModels.jl** — optimization to be **competitive** on fair harness cases | **~58%** | 2026-07-06 Windows reference ([`benchmarks/fair-rust-julia-reference-2026-07-06.json`](benchmarks/fair-rust-julia-reference-2026-07-06.json)): random-intercept cases within ~**2–3×** of Julia (down from ~**5–9×**); nested ~**8×** (was ~**28×**); crossed still ~**19×**. Shipped: cached [`LmmData`](src/math.rs) in θ search, precomputed `Z^T X` / `Z^T y`, deviance-only eval, intercept-only diagonal-Λ path. **Next:** LDL symbolic reuse, gradient-based θ optimizer, structure-aware crossed/nested solves. **Target:** within ~**2×** on all six fair cases. |

## Weighted “overall” (illustrative only)

Simple mean of the thirteen percentages:  
(94 + 88 + 87 + 86 + 92 + 97 + 93 + 84 + 98 + 90 + 76 + 35 + 58) ÷ 13 = 1090 ÷ 13 ≈ **83.8%**.

So the rough figure is **~84%** toward a hypothetical “full statistics stack with universal parity **and competitive native fit speed**.” Random-intercept throughput is near the ~2× target on the reference workstation, but **crossed** and **nested** fair cases still block calling axis (3) done — [`README.md`](README.md) describes the crate as usable today, but **performance-sensitive Rust workflows** with complex RE layouts should still benchmark on their data.
...
| Benchmarks / throughput | [`BENCHMARKS.md`](BENCHMARKS.md); [`scripts/run_fair_rust_julia_benchmark.py`](scripts/run_fair_rust_julia_benchmark.py); [`benchmarks/fair-rust-julia-reference-2026-07-06.json`](benchmarks/fair-rust-julia-reference-2026-07-06.json) |
