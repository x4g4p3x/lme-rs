...
**Versions checked:** `lme-rs` **0.1.12** (root [`Cargo.toml`](Cargo.toml)); Python extension **`lme_python` 0.1.12** ([`python/Cargo.toml`](python/Cargo.toml)).
...
| 7 | **Cross-language validation** — [`comparisons/`](comparisons/), JSON/CSV fixtures, Rust tests | **94%** | Golden parity includes `ssfpl` / `ssasympoff` / `ssasymporig`, CBPP AGQ-7, sleepstudy profile CIs; `SSbiexp`/`SSweibull` smoke-only (R `nlmer` DGP unstable). |
