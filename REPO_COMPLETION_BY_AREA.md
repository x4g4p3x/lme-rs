...
| 13 | **LMM fit throughput vs MixedModels.jl** — optimization to be **competitive** on fair harness cases | **~65%** | **Prior** ([`76fdb61`](benchmarks/fair-rust-julia-reference-2026-07-06.json)): random-intercept ~**2–3×** Julia; nested ~**8×**; crossed ~**19×**. **2026-07-07** ([OPTIMIZATION.md](OPTIMIZATION.md)): reused sparse LDL + p = 2 hot finish — crossed ~**8×**, nested ~**2.5×**, random-intercept ~**2.4×** on the same workstation. **Target:** within ~**2×** on all six fair cases. |
...
| **Competitive LMM fit vs MixedModels.jl** | **~65%** | Same as summary row 13. Prior `76fdb61` + 2026-07-07 intercept LDL pass ([OPTIMIZATION.md](OPTIMIZATION.md)). Completion criterion: fair-harness medians within ~**2×** of Julia on all six reference cases without breaking lme4 parity tests. |
