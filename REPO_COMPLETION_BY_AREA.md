...
| 13 | **LMM fit throughput vs MixedModels.jl** — optimization to be **competitive** on fair harness cases | **~70%** | **Prior** ([`76fdb61`](benchmarks/fair-rust-julia-reference-2026-07-06.json)): random-intercept ~**2–3×** Julia; nested ~**8×**; crossed ~**19×**. **2026-07-07** ([OPTIMIZATION.md](OPTIMIZATION.md)): LDL reuse + θ-search — crossed ~**8×**, nested ~**2.3×**. **2026-07-08**: [blocked augmented Cholesky](src/intercept_blocked.rs) (MixedModels `updateL!` layout) — crossed ~**5×**, nested ~**2.4×**, random-intercept ~**2.6×** on the same workstation. **Target:** within ~**2×** on all six fair cases. |
...
| Topic | Status | Notes |
|:------|:-------|:------|
| **Nonlinear mixed models** (`nlmer`-style) | **~87%** | [`nlmer`](src/nlmm/mod.rs) + Python `nlmer()`; built-in `SSlogis` / `SSasymp` / `SSfol` / `SSmicmen` / `SSgompertz`; [`CustomNlmmMean`](src/nlmm/mean_fn.rs) / [`nlmer_with_mean`](src/nlmm/mod.rs) (Rust + Python `nlmer_with_mean`); `selfStart` when `start` is empty (built-in means only); population/conditional [`predict`](src/nlmm/predict.rs); scalar and multivariate RE on one grouping factor; scalar AGQ (`NlmerOptions::n_agq`, Python `n_agq` when `k = 1`). Not yet: full `stats::SS*` catalog, multivariate AGQ, AGQ inside the θ optimizer (same caveat as GLMM). |
...
| **Competitive LMM fit vs MixedModels.jl** | **~70%** | Same as summary row 13. Prior `76fdb61` + 2026-07-07 LDL/θ-search + **2026-07-08 blocked Cholesky** ([OPTIMIZATION.md](OPTIMIZATION.md)). Completion criterion: fair-harness medians within ~**2×** of Julia on all six reference cases without breaking lme4 parity tests. |
