...
- Replaced the vendored `fiasto` parser with a panic-free, typed Wilkinson parser in `src/formula.rs`; lme-rs no longer converts parser JSON or carries a path dependency that blocks crates.io publication. Nested random effects now support arbitrary depth, and `0 + x` fixed-effect formulas are accepted.
- Optimized native parsing with borrowed lexer tokens, single-pass lexer metadata, inline common-case buffers, compact typed column-role flags, `AHashMap` output lookup, allocation-free additive-term visiting, pre-sized collections, constant-time generated-column deduplication, and non-duplicated random-effect metadata. Criterion successful-parse cases improved by 58–72%, with representative random-slope, nested, and crossed formulas below 0.7 µs on the recorded Windows workstation. See the [optimization experiment record](OPTIMIZATION.md#native-formula-parser-optimization-2026-08-10).
- Formula parsing now rejects transform and interaction-only syntax that the design-matrix builder cannot materialize (`poly(x, 2)`, `log(x)`, `offset(log(x))`, and fixed `a:b`) instead of returning misleading partial metadata.
- Random-effect matrix construction now preserves declaration boundaries: `||` creates independent variance blocks, and `(1 | g) + (0 + x | g)` is no longer silently merged into correlated `(1 + x | g)`.
...
- Malformed Wilkinson formulas return parser errors without unwinding or aborting, including the former `Rey ~ 1 (+ (1 |` regression seed.
- `cargo fmt` in CI/hooks formats only the `lme-rs` package.
...
- LMM **simple fixed-effects fast path**: [`try_build_simple_x_matrix`](src/model_matrix.rs) for `y ~ 1`, `y ~ x`, and `y ~ 1 + x` skips the generic formula loop on fair-harness fixtures. `prepare_lmer` on `crossed_20k` **~4.2 ms**; `random_intercept_10k` still **beats Julia** (~1.3 ms vs ~1.5 ms, 20 repeats). See [OPTIMIZATION.md § Prepare ownership pass](OPTIMIZATION.md#prepare-ownership-pass-2026-07-08-continued) and [BENCHMARKS.md § 2026-07-08 simple X](BENCHMARKS.md#fair-rust-julia-2026-07-08-simple-x).
...
- Design-matrix support for **`FixedEffect`** / **`InteractionTerm`** parser roles and two-way categorical interactions (`y ~ a * b`).
