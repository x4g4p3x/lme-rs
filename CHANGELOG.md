...
- **Group-structure-preserving CV** — [`cv_grouped`](src/cv.rs) splits by grouping units (all rows for one subject stay in train or test), fits each training fold with `prepare_lmer` + `fit_prepared`, and returns out-of-fold population predictions plus RMSE/MAE. Rust tests in [`tests/test_cv.rs`](tests/test_cv.rs); Python in [`python/tests/test_cv.py`](python/tests/test_cv.py). Documented in [GUIDE.md § Repeated fits and cross-validation](GUIDE.md#repeated-fits-and-cross-validation).
