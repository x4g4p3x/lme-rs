...
def test_lmer_sleepstudy():
    ...
    assert (
        abs(cond_preds[0] - preds[0]) > 0.1
    )  # Subject level effect should make it differ from population
...
def test_glmer_binomial():
    ...
    model = lme_python.glmer(
        "y ~ period2 + period3 + period4 + (1 | herd)", data=df, family_name="binomial"
    )
...
def test_conditional_response_unseen_levels():
    ...
    new_level_df = pl.DataFrame(
        {"TICKS": [10.0], "YEAR": [2000.0], "HEIGHT": [1.2], "BROOD": ["UNSEEN_BROOD_999"]}
    )
...
def test_invalid_family():
    ...
    with pytest.raises(ValueError, match="Unsupported or invalid family"):
        lme_python.glmer(
            "Reaction ~ Days + (Days | Subject)", data=df, family_name="invalid_family"
        )
...
def test_conditional_predictions_unseen_levels():
    ...
    new_level_df = pl.DataFrame(
        {"Reaction": [250.0], "Days": [0], "Subject": ["UNSEEN_SUBJECT_999"]}
    )
