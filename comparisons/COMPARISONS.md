...
| GLMM, CBPP cloglog / Gamma dyestuff | Manifest `cbpp_binomial_cloglog`, `gamma_dyestuff_log_laplace` | Non-canonical cloglog link; Gamma log-link mean, residual φ, and Batch RE θ vs lme4. |
...
| R Wilkinson / RE edge-case matrix | [`tests/test_r_edge_case_matrix.rs`](../tests/test_r_edge_case_matrix.rs) vs [`tests/data/r_edge_case_matrix.json`](../tests/data/r_edge_case_matrix.json) | Fit-level lme4 parity for `||`, `I()`, `sqrt()`, `poly()` / `ns()`, `0 + factor`, and nested `batch/cask`. Generator: [`tests/generate_r_edge_case_matrix.R`](../tests/generate_r_edge_case_matrix.R). |
...
| Other fixtures | e.g. [`tests/data/random_slopes.json`](../tests/data/random_slopes.json), [`tests/data/penicillin.json`](../tests/data/penicillin.json), [`tests/data/sleepstudy_confint_profile.json`](../tests/data/sleepstudy_confint_profile.json), [`tests/data/sleepstudy_confint_profile_vc.json`](../tests/data/sleepstudy_confint_profile_vc.json) | Additional REML / design-matrix / profile-CI (β and `.sig01`/`.sigma`) checks in the test suite. |

AGQ-specific checks (scalar and product quadrature, `n_agq > 1`) live in GLMM tests (including golden `cbpp_binomial_agq7` and a vector-RE Poisson smoke), [`tests/test_nlmm_agq.rs`](../tests/test_nlmm_agq.rs) for `nlmer` (scalar Orange and multivariate `Asym + xmid | Tree`), and in shared quadrature helpers in [`src/quadrature.rs`](../src/quadrature.rs): they assert finite deviances and, for CBPP at a fixed θ, Laplace vs AGQ marginal deviance within **5%** relative scale. Full θ-path AGQ-in-θ parity for CBPP at `nAGQ = 7` is locked in the golden manifest.

### `nlmer` — `SSpower` (MATLAB Curve Fitter `power2`)

Grouped calibration-style nonlinear mean **μ = a·x^b + c** (`y ~ SSpower(x, a, b, c) ~ c|group` in `lme-rs`).

| Engine | Status |
|:-------|:-------|
| **`lme-rs` / `lme_python`** | Built-in `SSpower` with `selfStart`; golden case `sspower_synthetic_self_start` in [`tests/data/golden_parity_manifest.json`](../tests/data/golden_parity_manifest.json); fixtures from [`tests/generate_nlmm_fixtures.R`](../tests/generate_nlmm_fixtures.R). |
| **R `lme4::nlmer`** | No `stats::SSpower`. Parity uses a **custom `selfStart` SSpower** (gradient attribute + log-linear `getInitial`) — see [`comparisons/nlmm_sspower.R`](nlmm_sspower.R) and the generator scripts. Raw `nlmer(y ~ a * x^b + c ~ …)` without that wrapper **fails** in lme4 because gradients are not defined. |
...
3. **Adaptive quadrature (`nAGQ > 1`)** — When `n_agq > 1`, **lme-rs** optimizes θ under AGQ deviance for scalar RE (matching `lme4::glmer` and `nlmer`), for vector RE via a product Gauss–Hermite rule per group, and for multiple RE terms when total `q` is small enough for joint AGQ. Larger crossed / high-dimensional RE stay Laplace-in-θ; AGQ may still be applied in the final PIRLS evaluation. The scripts [`comparisons/glmm_cbpp.R`](glmm_cbpp.R), [`glmm_cbpp.py`](glmm_cbpp.py), and [`glmm_cbpp.rs`](glmm_cbpp.rs) document both Laplace and higher `nAGQ` / `n_agq` runs side by side.
