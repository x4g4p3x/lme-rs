...
fn test_predict_conditional_unseen_levels() -> anyhow::Result<()> {
    ...
    assert!(cond_reject.is_err(), "Expected error for unknown level when allow_new_levels is false");
    ...
    assert!(cond_allow.is_ok(), "Expected success for unknown level when allow_new_levels is true");
