...
fn test_lm_perfect_fit_zero_residuals() {
    ...
    for &r in fit.residuals.iter() {
        assert!(r.abs() < 1e-12, "Residual should be ~0 for perfect fit, got {}", r);
    }
...
fn test_lm_display_formatting() {
    ...
    assert!(summary.contains("Scaled residuals:"), "lm display should have scaled residuals");
    assert!(summary.contains("Fixed effects:"), "lm display should have fixed effects");
}
...
fn test_lm_single_predictor() {
    ...
    assert!((fit.coefficients[0] - 2.0).abs() < 1e-10, "Intercept should be 2.0");
    assert!((fit.coefficients[1] - 3.0).abs() < 1e-10, "Slope should be 3.0");

    // Fitted values should exactly match y
    for i in 0..4 {
        assert!((fit.fitted[i] - y[i]).abs() < 1e-10, "Fitted[{}] mismatch", i);
    }
}
...
fn test_intercept_only_predict_is_constant() {
    ...
    assert!((preds[0] - preds[1]).abs() < 1e-10, "Intercept-only model should give constant predictions");
    assert!((preds[0] - preds[2]).abs() < 1e-10, "Intercept-only model should give constant predictions");
    
    // The prediction should match the fitted intercept
    assert!((preds[0] - fit.coefficients[0]).abs() < 1e-10,
        "Population pred should equal intercept: {} vs {}", preds[0], fit.coefficients[0]);
}
...
fn test_conditional_predict_intercept_only() {
    ...
    assert!(diff > 0.1, "Conditional should differ from population for known subject, diff = {}", diff);
}
...
fn test_fitted_plus_residuals_equals_y() {
    ...
    for i in 0..fit.num_obs {
        ...
        assert!((reconstructed - y_i).abs() < 1e-8,
            "y[{}] = {} but fitted + residuals = {}", i, y_i, reconstructed);
    }
}
...
fn test_multiple_subjects_conditional() {
    ...
    assert!((cond[0] - cond[1]).abs() > 0.01,
        "Conditional preds for different subjects should differ: {} vs {}", cond[0], cond[1]);
}
