...
fn test_fixed_effects_precision() {
    ...
    assert!(
        (beta0 - 251.4051).abs() < 0.05,
        "Intercept: expected ~251.405, got {}",
        beta0
    );
    assert!(
        (beta1 - 10.4673).abs() < 0.05,
        "Days slope: expected ~10.467, got {}",
        beta1
    );
    ...
    assert!(
        (se0 - 6.8246).abs() < 1.0,
        "SE(Intercept): expected ~6.82, got {}",
        se0
    );
    assert!(
        (se1 - 1.5423).abs() < 0.5,
        "SE(Days): expected ~1.54, got {}",
        se1
    );
}
...
fn test_residual_variance_precision() {
    ...
    assert!(
        (sigma2 - 654.94).abs() < 200.0,
        "Residual variance: expected ~654.94, got {}",
        sigma2
    );
    ...
    assert!(
        sigma > 15.0 && sigma < 40.0,
        "Residual std dev: expected ~25.6, got {}",
        sigma
    );
}
...
fn test_variance_components_precision() {
    ...
    assert!(
        !subject_variances.is_empty(),
        "Should have Subject variance components"
    );
...
fn test_deviance_precision() {
    ...
    assert!(
        (dev - 1763.9).abs() < 25.0,
        "REML deviance: expected ~1763.9 (±25 for optimizer differences), got {}",
        dev
    );
}
...
fn test_information_criteria_precision() {
    ...
    assert!(
        (ll - (-dev / 2.0)).abs() < 1e-10,
        "logLik should equal -deviance/2"
    );

    // BIC should be > AIC for n=180 (since ln(180) ≈ 5.19 > 2)
    assert!(
        bic > aic,
        "BIC ({}) should be > AIC ({}) for n=180",
        bic,
        aic
    );

    println!(
        "AIC: {:.1}, BIC: {:.1}, logLik: {:.1}, deviance: {:.1}",
        aic, bic, ll, dev
    );
}
...
fn test_ranef_values_nonzero() {
    ...
    assert!(
        sum_abs > 1.0,
        "Sum of |ranef| should be substantial, got {}",
        sum_abs
    );
}
...
fn test_conditional_vs_population_magnitude() {
    ...
    assert!(
        (pop[0] - 251.4).abs() < 1.0,
        "Pop pred at Day 0: expected ~251.4, got {}",
        pop[0]
    );

    // Conditional should differ from population
    let total_diff: f64 = pop
        .iter()
        .zip(cond.iter())
        .map(|(p, c)| (p - c).abs())
        .sum();
    assert!(
        total_diff > 0.1,
        "Conditional should meaningfully differ from population, total diff = {}",
        total_diff
    );
    ...
    for i in 0..3 {
        assert!(
            cond[i] > 100.0 && cond[i] < 500.0,
            "Conditional pred[{}] = {} is out of reasonable range",
            i,
            cond[i]
        );
    }
...
fn test_offset_parity() {
    ...
    let fit = lmer(
        "Reaction ~ Days + offset(OffsetCol) + (Days | Subject)",
        &df,
        true,
    )
    .unwrap();
    ...
    assert!(
        (beta0 - 251.405).abs() < 0.05,
        "Intercept with offset: expected ~251.405, got {}",
        beta0
    );

    // The slope for Days should be exactly 10.0 less than the original fit (10.467 - 10.0 = 0.467)
    assert!(
        (beta1 - 0.467).abs() < 0.05,
        "Days slope with offset: expected ~0.467, got {}",
        beta1
    );
    ...
    assert!(
        (sigma2 - 654.94).abs() < 200.0,
        "Residual variance with offset: expected ~654.94, got {}",
        sigma2
    );
}
#[test]
fn test_ml_vs_reml_deviance() {
    ...
    assert!(
        (dev_reml - dev_ml).abs() > 1.0,
        "REML ({}) and ML ({}) deviance should differ",
        dev_reml,
        dev_ml
    );
    ...
    for i in 0..fit_reml.coefficients.len() {
        assert!(
            (fit_reml.coefficients[i] - fit_ml.coefficients[i]).abs() < 1.0,
            "Beta[{}]: REML={}, ML={} should be similar",
            i,
            fit_reml.coefficients[i],
            fit_ml.coefficients[i]
        );
    }
}
