...
fn test_satterthwaite_anova_numerical_parity() {
    ...
    let r_anova_f = r_data["outputs"]["sat_anova_f"].as_array().expect("No Satterthwaite anova F found in JSON");
    let r_anova_p = r_data["outputs"]["sat_anova_p"].as_array().expect("No Satterthwaite anova P found in JSON");
    let r_anova_ndf = r_data["outputs"]["sat_anova_ndf"].as_array().expect("No Satterthwaite anova NumDF found in JSON");
    let r_anova_ddf = r_data["outputs"]["sat_anova_ddf"].as_array().expect("No Satterthwaite anova DenDF found in JSON");
    ...
    let anova_res = fit.anova(DdfMethod::Satterthwaite).expect("Failed to build Satterthwaite ANOVA table");
    ...
    assert!((rust_f - r_f).abs() < tol, "Satt ANOVA F mismatch on effect Days: Rust={}, R={}", rust_f, r_f);

    let rust_p = anova_res.p_value[0];
    let r_p = r_anova_p[0].as_f64().unwrap();
    assert!((rust_p - r_p).abs() < tol, "Satt ANOVA P mismatch on effect Days: Rust={}, R={}", rust_p, r_p);

    let rust_ndf = anova_res.num_df[0];
    let r_ndf = r_anova_ndf[0].as_f64().unwrap();
    assert!((rust_ndf - r_ndf).abs() < tol, "Satt ANOVA NumDF mismatch on effect Days: Rust={}, R={}", rust_ndf, r_ndf);

    let rust_ddf = anova_res.den_df[0];
    let r_ddf = r_anova_ddf[0].as_f64().unwrap();
    assert!((rust_ddf - r_ddf).abs() < tol, "Satt ANOVA DenDF mismatch on effect Days: Rust={}, R={}", rust_ddf, r_ddf);
}
